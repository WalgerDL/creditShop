...
public class OfferConsumer {
    ...
    private final ApprovedProducer approvedProducer;
    ...
    public void msgListener(ConsumerRecord<String, Object> record) {
        try {
            ...
            if (approved.isCompleted()) {
                log.info("Offer:= Кредит на сумму:" + String.format("%.2f", creditOffer.getAmount()) + " одобрен!");
                approved.setMessage("Кредит на сумму:" + String.format("%.2f", creditOffer.getAmount()) + " одобрен!");
            } else {
                log.info("Offer:= Кредит на сумму:" + String.format("%.2f", creditOffer.getAmount()) + " не может быть предоставлен!");
                approved.setMessage("Кредит на сумму:" + String.format("%.2f", creditOffer.getAmount()) + " не может быть предоставлен!");
            }

            //отправка сообщения
            approvedProducer.produceApproved(approved, "k_" + creditOffer.getId());
            log.info("=========kafka scoring ok!=========");
